...
public class SilverDagger extends MetalDagger{
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        ...
        tooltip.add(Text.literal("对亡灵生物给予最后一击后,获得2.5秒的再生I").formatted(Formatting.AQUA));
    }
    ...
    public void postDamageEntity(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if(target.isDead() && target.getType().isIn(EntityTypeTags.UNDEAD))
            attacker.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION,50));
    }
